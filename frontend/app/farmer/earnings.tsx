...
export default function EarningsScreen() {
  ...
  return (
    <View style={styles.container}>
      {/* Analytics header - Sticky at top */}
      <View style={styles.headerAnalytics}>
        {/* Summary cards */}
        ...
        {data.recent.map((order: any) => (
          <View key={order.id} style={styles.transactionCard}>
            <View style={{ flex: 1, paddingRight: 8 }}>
              <Text style={styles.txProduct} numberOfLines={1}>{order.product_name}</Text>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.colors.background, padding: 16, paddingBottom: 0 },
  headerAnalytics: { marginBottom: 4 },
  scrollList: { flex: 1 },
  scrollListContent: { paddingBottom: 32 },
  ...
  chartCard: { backgroundColor: "#fff", borderRadius: theme.borderRadius.md, padding: 16, marginBottom: 16, ...theme.shadow.sm },
  ...
  txRight: { alignItems: "flex-end", minWidth: 70 },
